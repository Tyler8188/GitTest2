 ...
 * @Description: 
 * @LastEditTime : 2021-02-23 15:41:27
 * @FilePath: \RomantoryApp\assets\Framework\Utility\dx\excuteNodeEvents.ts
 */
export function excuteNodeEvents(events: cc.Component.EventHandler[], evt?: any): void {
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (handler.component == null || handler.component == "") {
            ...
            target = handler.target.getComponent(handler.component);
        }
        ...
        if (fun.length == 0) {
            ...
            fun.apply(target, [evt]);
        }
        else if (fun.length == 2) {
            fun.apply(target, [evt, handler.customEventData]);
        }
    }
}
